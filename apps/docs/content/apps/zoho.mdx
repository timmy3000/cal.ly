...
```
<Cal.ly URL>/api/integrations/zohocrm/callback
```

Replace `<Cal.ly URL>` with your application URL.
...
10. **Integration is Ready** - Your ZohoCRM integration can now be easily added in the Cal.ly settings.
...
```
<Cal.ly URL>/api/integrations/zohocalendar/callback
```
...
5. **Save Credentials in Cal.ly Admin Panel** - Copy the Client ID and Client Secret to your app keys in the Cal.ly admin panel at:

```
<Cal.ly>/settings/admin/apps
```
...
```
<Cal.ly URL>/api/integrations/zoho-bigin/callback
```
...
7. **Complete Zoho Bigin Integration** - Your Zoho Bigin integration is now ready and can be added from the Cal.ly app store.

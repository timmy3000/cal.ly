...
```
<Cal.ly URL>/api/integrations/googlecalendar/callback
<Cal.ly URL>/api/auth/callback/google
```

Replace `<Cal.ly URL>` with the URL where your application runs.
...
### Adding Google Calendar to Cal.ly App Store

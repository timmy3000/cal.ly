...
This file contains domain knowledge about the Cal.ly product and codebase. For coding guidelines and rules, see [`rules/`](rules/).
...
### Cal.ly Event Identification

Cal.ly events in Google Calendar can be identified by checking if the iCalUID ends with `@Cal.ly` (e.g., `2GBXSdEixretciJfKVmYN8@Cal.ly`). This identifier is used to distinguish Cal.ly bookings from other calendar events for data storage and privacy purposes.
...
Workflows and webhooks are two completely separate features in Cal.ly with different implementations and file structures:
